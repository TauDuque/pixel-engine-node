...
describe("Hexagonal Task Integration Tests", () => {
  let app: App;
  ...
  describe("POST /api/tasks", () => {
    it("should create a task using hexagonal architecture with valid image path", async () => {
    ...
    it("should process image asynchronously using hexagonal architecture and update task status", async () => {
      const testImagePath = path.join(__dirname, "../fixtures/more.png");
      ...
      // Create task using hexagonal architecture
    ...
    it("should return 400 for missing imagePath in hexagonal architecture", async () => {
      const response = await request(app.getApp())
        .post("/api/tasks")
        .send({})
        .expect(400);
    ...
    it("should return 400 for invalid imagePath type in hexagonal architecture", async () => {
      const response = await request(app.getApp())
        .post("/api/tasks")
        .send({ imagePath: 123 })
        .expect(400);
    ...
    it("should return 400 for non-existent image file in hexagonal architecture", async () => {
      const response = await request(app.getApp())
        .post("/api/tasks")
  ...
  describe("GET /api/tasks/:taskId", () => {
    it("should return task details for valid taskId using hexagonal architecture", async () => {
      // First create a task using hexagonal architecture
      const testImagePath = path.join(__dirname, "../fixtures/more.png");
      ...
      // Then get the task using hexagonal architecture
      const response = await request(app.getApp())
        .get(`/api/tasks/${taskId}`)
        .expect(200);
    ...
    it("should return 404 for non-existent taskId in hexagonal architecture", async () => {
    ...
    it("should return 400 for invalid taskId format in hexagonal architecture", async () => {
      const response = await request(app.getApp())
  ...
  describe("Hexagonal Architecture Validation", () => {
    it("should demonstrate separation of concerns between original and hexagonal routes", async () => {
      const testImagePath = path.join(__dirname, "../fixtures/more.png");
      ...
      // Create task using original architecture
      const originalResponse = await request(app.getApp())
        .post("/api/tasks")
        .send({ imagePath: testImagePath })
        .expect(201);

      // Create task using hexagonal architecture
      const hexagonalResponse = await request(app.getApp())
        .post("/api/tasks")
        .send({ imagePath: testImagePath })
        .expect(201);

      // Both should work independently
      expect(originalResponse.body.success).toBe(true);
      expect(hexagonalResponse.body.success).toBe(true);
      expect(originalResponse.body.data.taskId).not.toBe(
        hexagonalResponse.body.data.taskId
      );
    });

    it("should maintain consistent API responses between architectures", async () => {
      const testImagePath = path.join(__dirname, "../fixtures/more.png");
      ...
      expect(originalResponse.body).toHaveProperty("success");
      expect(originalResponse.body).toHaveProperty("data");
      expect(originalResponse.body).toHaveProperty("message");

      expect(hexagonalResponse.body).toHaveProperty("success");
      expect(hexagonalResponse.body).toHaveProperty("data");
      expect(hexagonalResponse.body).toHaveProperty("message");

      // Data structure should be consistent
      expect(originalResponse.body.data).toHaveProperty("taskId");
      expect(originalResponse.body.data).toHaveProperty("status");
      expect(originalResponse.body.data).toHaveProperty("price");

      expect(hexagonalResponse.body.data).toHaveProperty("taskId");
      expect(hexagonalResponse.body.data).toHaveProperty("status");
      expect(hexagonalResponse.body.data).toHaveProperty("price");
    });
  });
});
